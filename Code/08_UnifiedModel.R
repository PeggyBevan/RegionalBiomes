...
#the best fitting model from previous epxeriment
...
#accounting for sample size difference
m2 = StatisticalModels::GLMER(modelData = data_LUth.25, responseVar = 'LogRichness',
                              fitFamily = 'gaussian', fixedStruct = "LandUse*RB_tnc", 
                              randomStruct = "(1|SS)+(1|SSB)", REML = T)
...
# Sampling - 24 studies per site ------------------------------------------

#notes - 13 studies per regional biome is a bit restrictive, it results in some land-use types being incomparable across regional biomes. So I will restrict the large biomes to 20 - this brings the range from 13 - 127 to 13-20. 
...
for (i in 1:nrow(ecoregs)) {
  ...
  if(ecoregs$n_studies[i] > 24) {
    ...
    studiessample = sample(rbstudies, 24)
    samp = data_LUth.25 %>%
      subset(RB_tnc == rb) %>%
    ...
    data_samp_min = rbind(data_samp_min, samp)
  } else {
    samp =  data_LUth.25 %>%
      subset(RB_tnc == rb)
    data_samp_min = rbind(data_samp_min, samp)
...
ecoregs_min <- data.frame('RB_tnc' = unique(data_samp_min$RB_tnc)) %>%
...
ecoregs_min$Biome_num<-as.numeric(ecoregs_min$Biome_num)
ecoregs_min<-arrange(ecoregs_min, Biome_num, Realm)

# calculate number of studies etc. for each RB
for (i in (1:nrow(ecoregs_min))) {
  rb = ecoregs_min$RB_tnc[i]
  tnc_code = paste(ecoregs_min$RB_tnc[i])
  ecoregs_min$n_studies[i] = n_distinct(data_samp_min$Study_name_taxa[data_samp_min$RB_tnc==rb])
  ecoregs_min$n_sources[i] = n_distinct(data_samp_min$Source_ID[data_samp_min$RB_tnc==rb])
  ecoregs_min$n_sites[i] = n_distinct(data_samp_min$SSBS[data_samp_min$RB_tnc==rb]) 
  ecoregs_min$n_ecoregions[i] = n_distinct(data_samp_min$Ecoregion[data_samp_min$RB_tnc==rb])
  ecoregs_min$n_taxa[i] = n_distinct(data_samp_min$my_taxa[data_samp_min$RB_tnc==rb])
  ecoregs_min$n_country[i] =n_distinct(data_samp_min$Country[data_samp_min$RB_tnc==rb])
  ecoregs_min$t_ecoregions[i] = n_distinct(tnc_ecoregions@data$ECO_NAME[tnc_ecoregions@data$RealmMHT==tnc_code])
  ecoregs_min$p_ecoregions[i] = round(ecoregs_min$n_ecoregions[i]/ecoregs_min$t_ecoregions[i], 2)
}

LU <- data_samp_min %>% count(RB_tnc, LandUse) %>% 
  spread(LandUse, n)
ecoregs_min<- merge(ecoregs_min, LU, by.x = 'RB_tnc', all.x = TRUE)
#create agriculture count
ecoregs_min$Agriculture <- rowSums(ecoregs_min[,14:16], na.rm = T)

Taxa = data_samp_min %>% count(RB_tnc, my_taxa) %>%
  spread(my_taxa, n)

ecoregs_min = merge(ecoregs_min, Taxa, by.x = 'RB_tnc', all.x = TRUE)


ecoregs_min$`Primary Vegetation`
ecoregs_min$Agriculture
 mean(ecoregs_min$n_sites)
 mean(ecoregs_min$n_studies)
 range(ecoregs_min$n_sites)
# #there is now a less than factor of 10 change
 range(ecoregs$n_sites)
range(ecoregs_min$n_studies)

#no taxonomic groups were lost from a regional biome during downsampling

#no regional biomes have become data deficient from the sampling procedure.
rm(tnc_ecoregions)
#the best fitting model from previous epxeriment
m2 = StatisticalModels::GLMER(modelData = data_samp_min, responseVar = 'LogRichness',
                              fitFamily = 'gaussian', fixedStruct = "LandUse*RB_tnc", 
                              randomStruct = "(1|SS)+(1|SSB)", REML = F)
summary(m2$model)
data_samp_min$RB_tnc = as.factor(data_samp_min$RB_tnc)
RBs <- data.frame(RB = unique(data_samp_min$RB_tnc))

preds_m <- apply(RBs, 1, FUN = function(rb) {
  nd <- data.frame(LandUse=factor(
    c('Primary Vegetation','Secondary Vegetation',"Plantation forest", "Cropland", "Pasture"),
...
preds2_m <- data.table::rbindlist(preds_m)  

#include sample size
for (i in 1:nrow(preds2_m)) {
  preds2_m$n[i] <- nrow(subset(data_samp_min, RB_tnc == preds2_m$RB[i] & LandUse == preds2_m$LU[i])) 
}
...
# preds2$Biome_num <- as.numeric(preds2$Biome_num) 
preds2_m<-arrange(preds2_m, Biome_num, Realm)
#remove ranks with less than 25 detections
preds2_m= subset(preds2_m, n > 25)
preds2_m$Realm = factor(preds2_m$Realm, levels = c('Afrotropic', 'Australasia','Indo-Malay', 'Nearctic', 'Neotropic', 'Palearctic'))

level_order_LU <- c("Primary Vegetation", "Secondary Vegetation", "Plantation forest", "Pasture","Cropland")

figm <- ggplot(preds2_m, aes(x = factor(LU, levels = level_order_LU), y = y, ymax = upper, ymin = lower, group = Realm, colour = Realm)) +
  theme_bw() +
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'grey') +
  geom_point(position = position_dodge(width = 0.6), size = 2) + 
  scale_colour_manual(values = c("#66C2A5", "#E78AC3", "#FC8D62","#A6D854","#8DA0CB", "#FFD92F")) +
  ...
  scale_x_discrete(name = 'Land Use', labels = c('PV', "SV", "PF", "Pa", "Cr")) +
  scale_y_continuous(name = 'Change in Species Richness (%)') +
  geom_text(aes(x = LU, group = Realm, label = signif(upper,3), y =100), hjust = 1.1, angle = 45,  data = preds2_m[preds2_m$upper > 100,], colour = 'black', position = position_dodge(width = 0.6), size = 2) +
  theme_bw() +
  theme(#panel.grid.major = element_blank(),
    #panel.grid.minor = element_blank(),
    strip.background = element_blank(),
...
ggsave('output/Eco_response/sampled_speciesrichness_casestudy.pdf', width = 7.5, height = 14, units = 'cm')

 ## Abundance model ---------------------------------------------------------

#there is a problem of one huge outlier in total abundance.

# outlier = data_samp_min[data_samp_min$Total_abundance > 10000,]
# outlier = outlier[!is.na(outlier$Source_ID),]
# unique(outlier$Study_name)
# unique(outlier$RB_tnc)
# 
# shapiro.test(data_samp_min$LogAbund[data_samp_min$Total_abundance < 10000])
# hist(data_samp_min$LogAbund[data_samp_min$Total_abundance < 10000])

#remove all values over 10,000 
data_samp_min$LogAbund[data_samp_min$Total_abundance >= 10000] = NA
hist(data_samp_min$LogAbund)

ma1 = StatisticalModels::GLMER(modelData = data_samp_min, responseVar = 'LogAbund',
                              fitFamily = 'gaussian', fixedStruct = "LandUse*RB_tnc", 
                              randomStruct = "(1|SS)+(1|SSB)", REML = F)
summary(ma1$model)

preds_ma <- apply(RBs, 1, FUN = function(rb) {
  nd <- data.frame(LandUse=factor(
    c('Primary Vegetation','Secondary Vegetation',"Plantation forest", "Cropland", "Pasture"),
    levels=levels(data_samp_min$LandUse)))
  nd$RB_tnc <- factor(rb, levels = levels(data_samp_min$RB_tnc))
...
preds2_ma <- data.table::rbindlist(preds_ma)  

#include sample size
for (i in 1:nrow(preds2_ma)) {
  preds2_ma$n[i] <- nrow(subset(data_samp_min[!is.na(data_samp_min$Total_abundance),], RB_tnc == preds2_ma$RB[i] & LandUse == preds2_ma$LU[i])) 
}

#add regional biome information for plotting
preds2_ma = left_join(preds2_ma, preds2_m[,c(1,2,10:13)], by = c('RB', 'LU'))
preds2_ma$Biome_num <- as.numeric(preds2_ma$Biome_num) 
preds2_ma<-arrange(preds2_ma, Biome_num, Realm)
#remove ranks with less than 25 detections
preds2_ma= subset(preds2_ma, n > 25)
preds2_ma$Realm = factor(preds2_ma$Realm, levels = c('Afrotropic', 'Australasia','Indo-Malay', 'Nearctic', 'Neotropic', 'Palearctic'))


figma <- ggplot(preds2_ma, aes(x = factor(LU, levels = level_order_LU), y = y, ymax = upper, ymin = lower, group = Realm, colour = Realm)) +
  theme_bw() +
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'grey') +
  ...
  geom_text(aes(x = LU, group = Realm, label = signif(upper,3), y =100), hjust = 1.1, angle = 45,  data = preds2_ma[preds2_ma$upper > 100,], colour = 'black', position = position_dodge(width = 0.7), size = 1.5) +
  coord_cartesian(ylim = c(-100,100)) +
  scale_x_discrete(name = 'Land Use', labels = c('PV', "SV", "PF", "Pa", "Cr")) +
...
ggsave('output/Eco_response/sampled_abundance_casestudy.pdf', width = 7.5, height = 14, units = 'cm')



# rm(tnc_ecoregions) #free up memory
...
# Taxon-regional biome model ----------------------------------------------
data_samp_min = droplevels(data_samp_min)

m1t = StatisticalModels::GLMER(modelData = data_samp_min, responseVar = 'LogRichness',
                               fitFamily = 'gaussian', fixedStruct = "LandUse:RB_tnc:my_taxa", 
                               randomStruct = "(1|SS)+(1|SSB)", REML = F)
summary(m1t$model)

#vertebrate
preds_t <- apply(RBs, 1, FUN = function(rb) {
  nd <- data.frame(LandUse=factor(rep(c('Primary Vegetation','Secondary Vegetation',"Plantation forest", "Cropland", "Pasture"), 3),
    levels=levels(data_samp_min$LandUse)))
  nd$RB_tnc <- factor(rb, levels = levels(data_samp_min$RB_tnc))
  nd$my_taxa = factor(c("Invertebrate", "Plant", "Vertebrate"), levels = levels(data_samp_min$my_taxa))
  nd$LogRichness = 0
  preds <- StatisticalModels::PredictGLMER(model = m1t$model, data = nd,
                                           se.fit = TRUE, seMultiplier = 1.96)
...
preds2_t <- data.table::rbindlist(preds_t)  

#include sample size
for (i in 1:nrow(preds2_t)) {
  preds2_t$n[i] <- nrow(subset(data_samp_min[!is.na(data_samp_min$Total_abundance),], RB_tnc == preds2_t$RB[i] & LandUse == preds2_t$LU[i])) 
}
...
preds2_t$Biome_num <- as.numeric(preds2_t$Biome_num) 
preds2_t<-arrange(preds2_t, Biome_num, Realm)
#remove ranks with less than 25 detections
preds2_t= subset(preds2_t, n > 25)
preds2_t$Realm = factor(preds2_t$Realm, levels = c('Afrotropic', 'Australasia','Indo-Malay', 'Nearctic', 'Neotropic', 'Palearctic'))


figt <- ggplot(preds2_t, aes(x = factor(LU, levels = level_order_LU), y = y, ymax = upper, ymin = lower, group = Realm, colour = Realm)) +
  theme_bw() +
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'grey') +
  ...
  geom_text(aes(x = LU, group = Realm, label = signif(upper,3), y =100), hjust = 1.1, angle = 45,  data = preds2_t[preds2_t$upper > 100,], colour = 'black', position = position_dodge(width = 0.75), size = 1) +
  coord_cartesian(ylim = c(-100,100)) +
  scale_x_discrete(name = 'Land Use', labels = c('PV', "SV", "PF", "Pa", "Cr")) +
  scale_y_continuous(name = 'Change in Species Richness (%)') +
  theme_bw() +
...
ggsave('output/Eco_response/downsampled_speciesrichness_taxon.pdf', width = 11.5, height = 11.5, units = 'cm')
...
ecoregs_t = arrange(ecoregs_min, Biome, Realm)
...
save_as_image(TS4, path = 'Output/TableS5.png')



# method 2 - minor sample size adjustment ---------------------------------

#having looked at the sample sizes, there is actually not that much discrepancy between biomes, as the reviwer suggested. 
