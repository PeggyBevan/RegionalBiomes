ModelSelectADMB<-function(all.data,responseVar,fitFamily,fixedFactors=
  ...
  repeat {
    ...
    mOld <- glmmadmb(formula = formula(call.old),data = model.data,
                     family = fitFamily,random = randomStruct)
    ...
    for (t in iTerms){
      ...
      if (substr(call.new,nchar(call.new),nchar(call.new))=="+"){
        call.new <- substr(call.new,1,nchar(call.new)-1)
      }
      
      mNew <- glmmadmb(formula = formula(call.new),data = model.data,
                       family = fitFamily,random = randomStruct)
      
      pVals<-c(pVals,anova(mOld,mNew)$Pr[2])
      Chis<-c(Chis,anova(mOld,mNew)$Deviance[2])
    ...
    if (substr(call.old,nchar(call.old),nchar(call.old))=="+"){
      call.old <- substr(call.old,1,nchar(call.old)-1)
    }
  ...
  if (dim(sig.terms)[1]>0){
    ...
    mBest <- glmmadmb(formula = formula(call.best),data = model.data,
                     family = fitFamily,random = randomStruct)
