...
PlotContEffects<-function(model,data,effects,otherContEffects=character(0),
  ...
  if ((length(effects)==1) && (is.null(byFactor))){
    ...
    if (!is.null(outDir)){
      png(paste(outDir,"/",effects," effect_",names(model.frame(model))[1],".png",
    ...
    if((!is.null(line.cols)) & (length(line.cols)!=length(levels(model.frame(model)[,byFactor])))){
      stop("Specified colours must be of the same number as the number of factor levels")
    }
    if((!is.null(line.types)) & (length(line.types)!=length(levels(model.frame(model)[,byFactor])))){
    ...
    for (l in levels(model.frame(model)[,byFactor])){
      newdat[,byFactor]<-factor(l,levels=levels(model.frame(model)[,byFactor]))
      ...
      if (byFactor=="LandUse"){
        if ("UI" %in% names(newdat)){
          try(newdat$UI<-intersect(grep(l,levels(model.frame(model)$UI),value=TRUE),
                                   grep("Minimal",levels(model.frame(model)$UI),value=TRUE)))
          newdat$UI<-factor(newdat$UI,levels=levels(model.frame(model)$UI))
        }
      }
      if (byFactor=="Taxon"){
        if ("LUTax" %in% names(newdat)){
          try(newdat$LUTax<-intersect(grep(l,levels(model.frame(model)$LUTax),value=TRUE),
                                      grep("Primary",levels(model.frame(model)$LUTax),value=TRUE)))
          newdat$LUTax<-factor(newdat$LUTax,levels=levels(model.frame(model)$LUTax))
        }
      }
    ...
    allLevels<-1:length(levels(model.frame(model)[,byFactor]))
    ...
    for (p in 1:nPanels){
      ...
      if (is.null(line.cols)){
        cols<-list()
        cols[1][[1]]<-paste(landUses$col[match(levels(model.frame(model)[,byFactor]),landUses$lu)])
      ...
      o<-na.omit(match(landUses$lu,levels(model.frame(model)[,byFactor])))
      o<-c(o,setdiff(1:length(levels(model.frame(model)[,byFactor])),o))
      
      if (plotUncertainty){
        i<-1
        for (l in levels(model.frame(model)[,byFactor])[effectsPerPlot[p][[1]]]){
          y.temp<-y[,c('x',l)]
    ...
    if (!is.null(outDir)){
      png(paste(outDir,"/",effects[1]," and ",effects[2]," effect_",names(model.frame(model))[1],".png",
                sep=""),width=22.86/2.54,height=12.57/2.54,units="in",res=1200)
    }
